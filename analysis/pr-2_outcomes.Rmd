...
## Data Analysis
Kruskal-Wallis one-way analysis of variance test was used to investigate whether there was a statistically significant difference between the behaviours reported by Puppy Raisers and the Training Outcome of the dogs.
C-BARQ Correlation with Training Outcome. The variables that showed p_value < 0.05 are below: 

```{r, echo = FALSE}

# Lizzy used an unequal variance t-test or Welch’s t-test 
...
vars = vars %>% mutate(outcome_p = mapply(FUN = stats, test = vars$Test, 
                var = vars$Variable, MoreArgs = list(prq, 'Outcome')))
vars$outcome_sign = ifelse(vars$outcome_p < 0.05, 1, 0)
vars %>% filter(outcome_sign == 1) %>% pull(Variable)
```


## Outcome
Below are the variables that showed statistically significant differences in behaviours considering different Training Outcomes ('Success', 'Fail').
...
lapply(vars_outcome[!str_detect(vars_outcome, "F_")], plot_categorical )

plot_continuous <- function(var){
    plot <- ggplot(prq, aes_string( x = 'Outcome', y = var)) +
            geom_boxplot(position = "dodge2", aes(colour = Outcome))
...
lapply(vars_outcome[str_detect(vars_outcome, "F_")], plot_continuous)
