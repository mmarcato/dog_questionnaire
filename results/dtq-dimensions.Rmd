...
```{r, echo = FALSE}
dtq = read.csv('data//2022-08-18-DTQ_MCPQ-R.csv', stringsAsFactors=TRUE)
colnames(dtq)
...
lr_breeds <- data.frame(transpose(lr_result[33]))
...
lr_results <- data.frame(do.call(rbind, lr_result[1:32]))
...
cat('Difference between Sex: \t', lr_results$p_value_1[32], '\t -> not significant')
...
colnames(data_2)
# CRITERIA 3
data_3 <- data_2 %>% select(Motivation.Persevering, Training, Amicability, Neuroticism, Outcome)
colnames(data_3)
```
...
correlationMatrix <- round(abs(cor(data_3 %>% select(-c("Outcome")), use = "pairwise.complete.obs")),2)
...
data_reduced <- data_3 %>% select(-c(highlyCorrelated)) 
...
The OR for PC1 is 2.16, this means that the probability of withdrawal increases by (2.16-1 = 1.16) 116% for every unit increase of PC1.
We are 95% confident that a unit increase in PC1 results in a 54 to 236% (1.54-1 = 0.54, 3.36-1 = 2.36) more odds of being withdrawn from training.
...
dc_pca_var$component <- mapvalues(dc_pca_var$component, from = 1:4, 
    to = c('PC1', 'PC2', 'PC3', 'PC4'))
