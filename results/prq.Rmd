...
lr_models <- lapply(data[-length(data)], function(x) glm(formula = Outcome ~ x, 
...
lr_results$significant_0.05 <- (lr_results$p_value_1 > 0) & (lr_results$p_value_1 < 0.05) & (lr_results$deviance_sign = TRUE)
lr_results$significant_0.1 <- (lr_results$p_value_1 > 0) & (lr_results$p_value_1 < 0.1) & (lr_results$deviance_sign = TRUE)
lr_results$significant_0.2 <- (lr_results$p_value_1 > 0) & (lr_results$p_value_1 < 0.2) & (lr_results$deviance_sign = TRUE)
print(colSums(lr_results[,12:14], na.rm = TRUE))
...
Removing features with more than 0.6 correlation (i.e. F_03)
```{r, echo = FALSE}
...
highlyCorrelated <- findCorrelation(correlationMatrix, cutoff=0.60, names = TRUE, exact = TRUE)
...
cat("Reduced Feature Set with p<0.05 and < 5% missing data and <70% correlated\t", dim(data_reduced))
```
...
plot_discrete <- function(var){
    ...
    plot1 <- ggplot(data_reduced, aes_string( x = var, fill = 'Outcome', width = 1)) + 
            geom_bar(position = "dodge") + xlab("Score") + 
