...
def factor(df):
    df['F_01 Trainability'] = score(df.iloc[:,[1, 2, 3, 4, 5, 6, 7, 8]] )
    df['F_02 Stranger-directed aggression'] = score(df.iloc[:,[10, 11, 12, 15, 16, 18, 20, 21, 22, 28]] )
    df['F_03 Owner-directed aggression'] = score(df.iloc[:,[9, 13, 14, 17, 19, 25, 30, 31]] )
    df['F_04 Dog rivalry'] = score(df.iloc[:,[32, 33, 34, 35]] )
    df['F_05 Stranger-directed fear'] = score(df.iloc[:,[36, 37, 39, 40]] )
    df['F_06 Nonsocial fear'] = score(df.iloc[:,[38, 41, 42, 44, 47, 48]] )
    df['F_07 Dog-directed aggression'] = score(df.iloc[:,[23, 24, 26, 29]] )
    df['F_08 Dog-directed fear'] = score(df.iloc[:,[45, 46, 52, 53]] )
    df['F_09 Touch sensitivity'] = score(df.iloc[:,[43, 49, 50, 51]] ) 
    df['F_10 Separation-related behaviour'] = score(df.iloc[:,[54, 55, 56, 57, 58, 59, 60, 61]] ) 
    df['F_11 Excitability'] = score(df.iloc[:,[62, 63, 64, 65, 66, 67]] ) 
    df['F_12 Attachment/attention-seeking'] = score(df.iloc[:,[68, 69, 70, 71, 72, 73]] ) 
    df['F_13 Chasing'] = score(df.iloc[:,[27, 74, 75, 76]] )
